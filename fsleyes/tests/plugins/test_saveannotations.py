...
def test_serialise_deserialise():
    ...
    with mock.patch('fsleyes.gl.textures.Texture2D', mock.MagicMock()):
        objs = [
            ...
            annotations.Arrow(         annot, 1, 2, 3, 4, honourZLimits=True),
            annotations.Rect(          annot, 1, 2, 3, 4, alpha=25, border=False),
            annotations.Ellipse(       annot, 1, 2, 3, 4, filled=False),
            annotations.TextAnnotation(annot, r'some text" \n ', 1, 2,
        ...
        annots       = {'X' : annot, 'Y' : annot, 'Z' : annot}
        deserialised = []
